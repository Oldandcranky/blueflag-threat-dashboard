...
app.get('/api/arc/:id', (req, res) => {
  ...
  const RECO_MAP = {
    ...
    'Terraform files with critical or high misconfigurations': {
      ...
      salesLine:'BlueFlag continuously monitors your repositories for IaC misconfigurations and surfaces them by severity and identity — so your team knows exactly who introduced the risk and which repositories to remediate first.'
    },
    'Unverified commit changes to terraform files': {
      ...
      salesLine:'BlueFlag tracks commit verification status across all infrastructure files and alerts the moment an unsigned change appears in a sensitive configuration path — giving you an audit-ready record of every IaC modification.'
    },
    // ── Secrets / Credentials ────────────────────────────────────────────────
    'Credential detected in code commit': {
      ...
      salesLine:'BlueFlag detects credential patterns in commits across all repositories the moment they are pushed — and attributes each exposure to the specific developer identity responsible, so you can act in minutes, not weeks.'
    },
    'Personal access tokens with write access': {
      ...
      salesLine:'BlueFlag maintains a live inventory of every personal access token across your organization — surfacing stale tokens with write access before attackers find them, and giving you one-click visibility into which identities need immediate rotation.'
    },
    ...
    'Code scans with critical or high vulnerabilities': {
      ...
      salesLine:'BlueFlag surfaces unresolved SAST findings by identity and repository, showing you exactly which developers are bypassing your scan gates and which codebases carry the highest unpatched risk right now.'
    },
    'Open source packages with critical vulnerabilities': {
      ...
      salesLine:'BlueFlag maps critical dependency vulnerabilities to the identities and repositories that introduced them — so instead of a flat list of CVEs, your team has a prioritized, owner-attributed action plan.'
    },
    'Open source packages with high vulnerabilities': {
      ...
      salesLine:'BlueFlag tracks dependency vulnerability debt over time, giving leadership a trending view that shows whether your OSS hygiene is improving or degrading — and the identity-level attribution needed to drive accountability.'
    },
    'Open source packages with low vulnerabilities': {
      ...
      salesLine:'BlueFlag keeps your full dependency posture visible without manual audits — automatically surfacing new findings as packages age and providing the evidence trail auditors need to validate your compliance posture.'
    },
    'Open source packages with GPL license': {
      ...
      salesLine:'BlueFlag scans for license risk across all active repositories and flags GPL introductions the moment they are committed — long before they surface in an M&A due-diligence review or legal audit.'
    },
    'Open source packages with LGPL license': {
      ...
      salesLine:'BlueFlag tracks license changes across your entire dependency graph and alerts your team when LGPL obligations are introduced — providing the documentation trail that satisfies enterprise customer security reviews.'
    },
    ...
    'Branch protection bypassed by administrators': {
      ...
      salesLine:'BlueFlag tracks every branch protection bypass event by identity and timestamps it — giving you a complete audit trail of each time a protection rule was circumvented and who was responsible.'
    },
    'Merged pull requests with check run failures': {
      ...
      salesLine:'BlueFlag identifies the specific identities responsible for overriding failed CI checks — giving you the accountability data needed to address patterns of policy bypass before they become a compliance or breach event.'
    },
    'Unverified commit changes to build configuration files': {
      ...
      salesLine:'BlueFlag monitors all changes to build configuration files and flags unverified commits in real time — the exact detection capability that would have surfaced the SolarWinds-style pipeline tampering before it reached production.'
    },
    // ── Identity / Access ────────────────────────────────────────────────────
    'Identities with multiple risky policy violations': {
      ...
      salesLine:'BlueFlag\'s identity risk scoring automatically surfaces high-concentration accounts like these — so your security team always has a prioritized, evidence-backed list of which identities require immediate intervention, without manual correlation across tools.'
    },
    'PRs approved by users with no prior commit history': {
      ...
      salesLine:'BlueFlag maps PR approval activity back to committer history across your entire organization — making ghost approvers and inactive reviewers immediately visible so you can clean up your review chain before it becomes a liability.'
    },
    'Mismatched committer and author names': {
      ...
      salesLine:'BlueFlag tracks commit author and committer identity across your entire organization, automatically surfacing mismatches that indicate spoofing or credential sharing — the kind of attribution gap that only becomes visible after a breach.'
    },
    ...
    'Suspicious activity on repository after long period of inactivity': {
      ...
      salesLine:'Have BlueFlag configured to monitor dormant repositories — it automatically alerts the moment an inactive repository returns to life, giving your team the earliest possible signal before malicious changes reach production.'
    },
    'Suspicious hour of user activity': {
      ...
      salesLine:'BlueFlag builds behavioral baselines for each developer identity and flags off-hours access automatically — no manual log analysis, no SIEM configuration required. This finding was surfaced by BlueFlag without any additional tooling.'
    },
    'Suspicious spike of activity on new repositories by identities': {
      ...
      salesLine:'BlueFlag monitors repository access velocity in real time and alerts on the bulk-access patterns that signal an active exfiltration attempt — the exact detection that limits your exposure window from days to minutes.'
    },
  };
    ...
    .map(({ p, r }, i) => {
      ...
      return `<div class="rec-item">
        ...
        <div>
          ...
          ${r.implication ? `<div class="rec-implication"><strong>Why it matters:</strong> ${r.implication}${r.salesLine ? `<div style="margin-top:8px;padding:7px 10px;background:#eff6ff;border-left:3px solid #1550FF;border-radius:0 4px 4px 0;color:#1550FF;font-size:12px;font-weight:600;font-style:normal">${r.salesLine}</div>` : ''}</div>` : ''}
